// MAINLAND — game.js — Freeze-proof edition
...
// Death overlay
let deathAlpha=0, deathActive=false;
...
const Game = {
  ...
  async start() {
    Audio.init();
    this.floorNumber=1; this.player=null;
    deathAlpha=0; deathActive=false;
    await this.loadFloor(1,false);
    Screens.show("game-screen"); Screens.hideAllOverlays();
  ...
  restart() {
    Object.assign(this, {
      state:"menu", floorNumber:1, dungeon:null, player:null,
      enemies:[], boss:null, npcs:[], shrine:null,
      items:new Map(), arrows:[], particles:[],
      activeNPC:null, bossIntro:false,
    });
    deathAlpha=0; deathActive=false;
    Dialogue.close();
  ...
  async loadFloor(n, carryPlayer=true) {
    ...
    const data = await safeFetch(`${API_BASE}/api/floor`, {
      floor_number:n, seed:Math.floor(Math.random()*999999)
    });

    if (!data) {
      // Floor load failed — just go back to menu
      console.error("Floor load failed");
      Screens.hideOverlay("loading-screen");
      this.state="menu"; Screens.show("menu-screen"); return;
    }
    ...
    // Fire-and-forget narrative — totally non-blocking
    safeFetch(`${API_BASE}/api/narrative`,{kind:"floor",floor_number:n,zone_name:data.zone_name})
      .then(d=>{if(d?.text)setTimeout(()=>UI.showNarrative(d.text),900);});
  ...
  loop() {
    ...
    try {
      ...
      // If update() throws, log and stay in current state — never freeze
  ...
  update() {
    ...
    for (const e of this.enemies) {
      if (e.hp<=0&&!e._xpGiven) {
        ...
        // Fire-and-forget death line
        safeFetch(`${API_BASE}/api/narrative`,{kind:"death",enemy_name:e.name})
    ...
    // Boss
    if (this.boss) {
      if (!this.bossIntro&&Math.abs(this.boss.gx-this.player.gx)<10&&Math.abs(this.boss.gy-this.player.gy)<10) {
        this.bossIntro=true; UI.showBoss(this.boss);
        Audio.boss(); Shake.trigger(12,20);
        safeFetch(`${API_BASE}/api/boss_taunt`,{boss_name:this.boss.name,lore:this.boss.lore,phase:1})
      ...
      if (this.boss.phase2Done&&!this.boss._phase2Announced) {
        ...
        UI.toast(`${this.boss.name} ENRAGES!`,"bad"); Shake.trigger(10,16);
        safeFetch(`${API_BASE}/api/boss_taunt`,{boss_name:this.boss.name,lore:this.boss.lore,phase:2})
          .then(d=>{if(d?.taunt)UI.toast(`"${d.taunt}"`,"bad");});
      }
      if (this.boss.hp<=0&&!this.boss._xpGiven) {
        ...
        Audio.kill(); Audio.levelup(); Shake.trigger(14,20); UI.hideBoss();
    ...
    if (this.items.has(ikey)) {
      const name=this.items.get(ikey); this.items.delete(ikey);
      this.player.pickItem(name);
      UI.toast(`Found ${name}!`,"info");
      safeFetch(`${API_BASE}/api/narrative`,{kind:"item",item_name:name,item_desc:ITEM_DEFS[name]?.desc||""})
    ...
    // ── STAIRS ── enemies all dead (hp<=0 counts, not animation done)
    const allEnemiesDead = this.enemies.every(e=>e.hp<=0) && (!this.boss||this.boss.hp<=0);
    const nearStairs =
      Math.abs(this.player.gx-this.dungeon.stairsPos.gx)<=1 &&
      Math.abs(this.player.gy-this.dungeon.stairsPos.gy)<=1;

    if (nearStairs && allEnemiesDead) {
    ...
    if (!this.player.isAlive && this.state==="playing") {
      this.state="dying"; deathActive=true;
      Audio.death(); Shake.trigger(15,30);
      setTimeout(()=>{
        deathActive=false; this.state="gameover";
  ...
  draw() {
    ...
    if (this.state==="playing") {
      ...
      ctx.font="11px 'Crimson Pro',serif"; ctx.textAlign="left";
  ...
  _doAttack() {
    ...
    const t=this.boss&&this.boss.hp>0?[...this.enemies,this.boss]:[...this.enemies];
    this.player.doAttack(t);
  },

  _shootArrow() {
    if (this.state!=="playing"||!this.player) return;
    const a=this.player.shootArrow();
    if (a) this.arrows.push(a); else UI.toast("No arrows — find a Quiver!","bad");
  },
  ...
  _tryShrine() {
    ...
    this.shrine.used=true; this.player.useShrine();
  ...
  _startDialogue(npc) {
    ...
    // Fetch deeper line from Gemini — fire and forget
    safeFetch(`${API_BASE}/api/dialogue`,{
      npc_name:npc.name, npc_role:npc.role,
      player_line:`Floor ${this.floorNumber}, level ${this.player?.level||1}, ${this.player?.kills||0} kills.`,
      history:npc.history.slice(-2),
      floor_number:this.floorNumber, player_level:this.player?.level||1,
    }).then(d=>{
  ...
  _endDialogue() {
    Dialogue.close(); this.activeNPC=null;
...
function handleKeyDown(e) {
  if (Game.state==="dialogue") {
    if (e.key==="e"||e.key==="E"||e.key==="Escape"){e.preventDefault();Game._endDialogue();}
    return;
  }
